...
Markdown 只是一种语法. 那么用来写 Markdown 的文本编辑器呢? 我推荐的是 **Typora** 或者 **Obsidian** & **Notion**
...
### 13. *注释 ["[^]" 后置, ^后面可以添加内容,中括号后面加: 作文末注释]
...
- **插入链接：ctrl+k** []中括号里面写文本()小括号里面写链接

- 无序列表：” **-** “+ 空格 或 “ **+** ” + 空格

- 有序列表：数字+“**.**”+空格

- `字体加灰(其实是代码)`: Ctrl + Shift + `
...
- 选中同一行相同格式的文字：ctrl+e
  ...
  - 无序列表缩进一级按下Tab键就会变成子级
    - 未输入内容时按下空格键就能删除列表

### 偏好设置里 Markdown拓展语法全开(一般只能本地文件使用)
